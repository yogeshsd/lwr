 ...
 */
	function deleteUser(username){
		...
		if(x){
			var request = $.ajax({
				url: "http://localhost:8080/lwr/rest/users/remove/"+username,
				type: "POST",
	...
	function saveUser(){
		...
		if( role == ''){
			...
			users["role"]=role;
			var request = $.ajax({
				url: "http://localhost:8080/lwr/rest/users/save?displayname="+displayName+"&username="+username+"&password="+password+"&role="+role,
				type: "POST",
				success: function(resp){
							var row = $("#"+username);
							if( row.length == 0){
								var row = "<tr onclick=\"selectUser(this)\" id=\""+username+"\"><td>"+displayName+"</td><td>"+username+"</td><td>"+password+"</td><td>"+role+"</td><td><img src=\"/lwr/images/sign-delete.png\" onclick=\"deleteUser(\'"+username+"\')\"></img></td></tr>";
	...
	function saveProfile(){
		...
		var request = $.ajax({
			url: "http://localhost:8080/lwr/rest/users/save?displayname="+displayName+"&username="+username+"&password="+password+"&role="+role+"&charttype="+chartType+"&refreshInterval="+refreshInterval,
			type: "POST",
			success: function(resp){
						$("#savetext").html(resp);
					},
